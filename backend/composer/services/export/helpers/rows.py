    ...
    for statement_alert in cs.statement_alerts.all():
        rows.append(
            Row(
                ...
                object_uri="",
                predicate_relationship=statement_alert.alert_type.name,
...
def get_origin_row(origin: AnatomicalEntity, review_notes: str, curation_notes: str):
    predicate_mapping = EXPORT_RELATIONSHIP_MAPPINGS[
        ExportRelationships.hasSomaLocatedIn
    ]
...
def get_destination_row(destination: Destination, total_vias: int):
    predicate_mapping = EXPORT_RELATIONSHIP_MAPPINGS[
        DESTINATION_PREDICATE_MAP[destination.type]
    ]
...
def get_via_row(via: Via):
    predicate_mapping = EXPORT_RELATIONSHIP_MAPPINGS[VIA_PREDICATE_MAP[via.type]]
...
def get_specie_row(specie: Specie):
    predicate_mapping = EXPORT_RELATIONSHIP_MAPPINGS[
        ExportRelationships.hasInstanceInTaxon
    ]
...
def get_sex_row(cs: ConnectivityStatement):
    predicate_mapping = EXPORT_RELATIONSHIP_MAPPINGS[
        ExportRelationships.hasBiologicalSex
    ]
...
def get_circuit_role_row(cs: ConnectivityStatement):
    predicate_mapping = EXPORT_RELATIONSHIP_MAPPINGS[
        ExportRelationships.hasCircuitRolePhenotype
    ]
...
def get_projection_row(cs: ConnectivityStatement):
    predicate_mapping = EXPORT_RELATIONSHIP_MAPPINGS[
        ExportRelationships.hasProjectionLaterality
    ]
...
def get_soma_phenotype_row(cs: ConnectivityStatement):
    predicate_mapping = EXPORT_RELATIONSHIP_MAPPINGS[
        ExportRelationships.hasSomaPhenotype
    ]
...
def get_phenotype_row(cs: ConnectivityStatement):
    predicate_mapping = EXPORT_RELATIONSHIP_MAPPINGS[
        ExportRelationships.hasAnatomicalSystemPhenotype
    ]
...
def get_projection_phenotype_row(cs: ConnectivityStatement):
    predicate_mapping = EXPORT_RELATIONSHIP_MAPPINGS[
        ExportRelationships.hasProjectionPhenotype
    ]
...
def get_functional_circuit_row(cs: ConnectivityStatement):
    predicate_mapping = EXPORT_RELATIONSHIP_MAPPINGS[
        ExportRelationships.hasFunctionalCircuitRolePhenotype
    ]
